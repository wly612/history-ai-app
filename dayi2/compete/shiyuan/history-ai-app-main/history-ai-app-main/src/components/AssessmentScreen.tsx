...
interface AssessmentScreenProps {
  ...
  onComplete: (incorrectIds: string[], sceneId?: string, result?: { totalAnswered: number; correctAnswered: number; incorrectAnswered: number }) => void;
  onBack?: () => void;
}
...
export const AssessmentScreen: React.FC<AssessmentScreenProps> = ({ sceneId: propSceneId, appState, onComplete, onBack }) => {
  ...
  useEffect(() => {
    if (propSceneId) {
      handleStartSceneQuiz(propSceneId);
    }
  }, [propSceneId]);
  ...
  const handleFinish = async () => {
    ...
    onComplete(incorrectIds, selectedSceneId || undefined, {
      ...
      incorrectAnswered: totalQuestions - correctCount,
    });
  };
