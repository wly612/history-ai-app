...
import { searchSimilarChunks, buildRAGContextForDialogue } from './ragService';
...
const RAG_THRESHOLD = parseFloat(process.env.RAG_THRESHOLD || '0.6');
const RAG_USE_HYBRID_SEARCH = process.env.RAG_USE_HYBRID_SEARCH !== 'false';
const RAG_USE_CACHE = process.env.RAG_USE_CACHE !== 'false';
...
export async function chatWithHistoricalFigure(
  ...
  if (options.useRAG !== false && RAG_ENABLED) {
    try {
      const searchResults = await searchSimilarChunks(message, {
        ...
        useCache: RAG_USE_CACHE,
        useHybridSearch: RAG_USE_HYBRID_SEARCH
      });
      ...
      if (searchResults.length > 0) {
        ...
        ragContext = buildRAGContextForDialogue(
          message,
          searchResults,
          npc.name,
          node ? `${node.year}年 ${node.title} - ${node.description}` : undefined
        );
        
        enhancedMessage = `用户问题：${message}\n\n${ragContext}`;
        console.log(`[RAG] Retrieved ${searchResults.length} chunks for query, avg similarity: ${(searchResults.reduce((sum, r) => sum + r.similarity, 0) / searchResults.length).toFixed(3)}`);
      } else {
  ...
  const systemPrompt = `${npc.systemPrompt}\n${context}\n请基于该时代背景和你的身份，用符合你性格的方式回答问题并给予修正反馈。保持沉浸感。${ragContext ? '\n\n你会获得相关的历史资料作为参考，请优先使用资料中的准确信息。如果资料与你的知识有冲突，以资料为准。' : ''}`;
