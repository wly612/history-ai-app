...
const AGENT_MODEL = process.env.AGENT_MODEL || 'deepseek-v4-flash';
const AGENT_FALLBACK_MODEL = process.env.AGENT_FALLBACK_MODEL || 'deepseek-v4-pro';
const AGENT_MAX_STEPS = Number(process.env.AGENT_MAX_STEPS || 12);
const AGENT_THINKING_TYPE = process.env.AGENT_THINKING_TYPE || 'disabled';
...
async function chatCompletion(messages: any[]) {
  const apiKey = process.env.AGENT_API_KEY || process.env.DEEPSEEK_API_KEY || process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error('Missing AGENT_API_KEY, DEEPSEEK_API_KEY, or GEMINI_API_KEY');
  }
  ...
  for (const model of models) {
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (AGENT_BASE_URL.includes('deepseek.com')) {
        payload.thinking = { type: AGENT_THINKING_TYPE };
      }
